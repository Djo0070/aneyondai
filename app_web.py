...
def init_chat_model():
    ...
    Tu te présentes comme ANEYOND.
    """
    model = genai.GenerativeModel('gemini-2.5-flash', system_instruction=system_prompt)
    return model.start_chat(history=[])

def analyze_image_with_gemini(image_data, prompt):
    """Envoie une image et un texte à Gemini Vision, retourne la réponse."""
    import google.generativeai as genai
    from PIL import Image
    import io

    genai.configure(api_key=os.getenv("GEMINI_API_KEY"))
    model = genai.GenerativeModel('gemini-2.0-flash')  # ou 'gemini-1.5-pro'
...
if firebase_cred_str:
    try:
        ...
        st.success("✅ Base de données connectée (historique disponible)")
    except Exception as e:
        st.warning(f"⚠️ Erreur Firestore : {e}. L'historique ne sera pas sauvegardé.")
        db = None
else:
    # Tentative de chargement depuis le fichier local (pour développement)
    ...
    if os.path.exists(cred_path):
        try:
            ...
            db = None
            st.warning("⚠️ Fichier serviceAccountKey.json invalide. Historique désactivé.")
    else:
        db = None
        st.warning("⚠️ Base de données non configurée. L'historique des conversations est désactivé.")


# ============================================
# FONCTIONS POUR L'HISTORIQUE DES CONVERSATIONS
# ============================================
def get_conversations(user_id):
    """Récupère toutes les conversations d'un utilisateur, triées par date décroissante."""
    if not user_id or db is None:
        return []
    convs = db.collection("users").document(user_id).collection("conversations").order_by("created_at", direction=firestore.Query.DESCENDING).stream()
...
def save_conversation(user_id, messages, conv_id=None):
    """Sauvegarde une conversation (crée ou met à jour)."""
    if not user_id or db is None:
        return None
    # Titre = premier message utilisateur (30 premiers caractères)
    ...
    if conv_id:
        conv_ref.document(conv_id).update(data)
        return conv_id
    else:
        data["created_at"] = firestore.SERVER_TIMESTAMP
        new_ref = conv_ref.document()
        new_ref.set(data)
        return new_ref.id
...
st.markdown("""
<style>
    /* Force les couleurs pour TOUS les éléments */
    * {
    ...
    .main, .stMarkdown, div[data-testid="stVerticalBlock"] {
        color: #FFFFFF !important;
    }

    /* Messages du chat – version ultra-robuste */
    ...
    [data-testid="chat-message-content"] p,
    [data-testid="chat-message-content"] span,
    [data-testid="chat-message-content"] div {
        color: white !important;
    }
...
with st.sidebar:
    ...
    if st.session_state.user is None:
        ...
        if st.button("➕ Nouvelle discussion", use_container_width=True):
            st.session_state.messages = [{"role": "assistant", "content": "👋 Bonjour ! Je suis **ANEYOND**. Comment puis-je vous aider ?"}]
            ...
            for conv in st.session_state.conversations:
                ...
                with col2:
                    if st.button("🗑️", key=f"del_{conv['id']}"):
                        db.collection("users").document(user_id).collection("conversations").document(conv["id"]).delete()
...
if menu == "💬 Chat":
    ...
    if "user" not in st.session_state or st.session_state.user is None:
        # Utilisateur non connecté : limiter à 5 messages
        if "guest_messages" not in st.session_state:
            st.session_state.guest_messages = 0
        if st.session_state.guest_messages >= 5:
            ...
            st.stop()  # Arrête l'exécution de la page Chat
    else:
    ...
    if "user" in st.session_state and st.session_state.user is not None:
        ...
        if "conversations" not in st.session_state:
            st.session_state.conversations = get_conversations(user_id) if db else []
    ...
    # Initialisation du client Groq (texte) et des messages
    ...
    with col1:
        if st.button("📎", help="Joindre un fichier"):
    ...
    if st.session_state.get("show_file_uploader", False):
        uploaded_file = st.file_uploader("Choisissez un fichier", type=["png", "jpg", "jpeg", "pdf", "txt"], key="file_uploader")
        if uploaded_file is not None:
            st.session_state.attached_file = uploaded_file
            st.success(f"Fichier '{uploaded_file.name}' attaché.")
            st.session_state.show_file_uploader = False
            st.rerun()

    # Caméra
    if st.session_state.get("show_camera", False):
        camera_photo = st.camera_input("Prenez une photo", key="camera")
        if camera_photo is not None:
            st.session_state.attached_file = camera_photo
            st.success("Photo attachée.")
            st.session_state.show_camera = False
            st.rerun()

    # Zone de saisie
    if prompt := st.chat_input("💭 Votre message...", key="main_chat_input"):
        # Vérifier s'il y a un fichier attaché
        attached = st.session_state.pop("attached_file", None)
        if "user" not in st.session_state or st.session_state.user is None:
            st.session_state.guest_messages += 1
        # Ajouter le message utilisateur à l'historique (sans l'image)
        st.session_state.messages.append({"role": "user", "content": prompt})
        with st.chat_message("user"):
            st.markdown(prompt)

        # Traitement de la réponse
        if attached and attached.type and attached.type.startswith("image"):
            # C'est une image : on appelle Gemini Vision
            try:
                import google.generativeai as genai
                from PIL import Image
                import io
                ...
                img = Image.open(io.BytesIO(attached.getvalue()))
                response = model.generate_content([prompt, img])
                reply = response.text
            except Exception as e:
                reply = f"❌ Erreur lors de l'analyse de l'image : {e}"
        else:
            # Pas d'image : appel à Groq (texte)
            try:
        ...
        if user_id and db is not None:
            if st.session_state.current_conv_id:
                save_conversation(user_id, st.session_state.messages, st.session_state.current_conv_id)
            else:
                new_id = save_conversation(user_id, st.session_state.messages)
